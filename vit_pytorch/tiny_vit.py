...
class PatchEmbed(nn.Module):
    def __init__(self, in_chans, embed_dim, resolution, activation):
        super().__init__()
        res_div = 4
        strides = [2, 2] if res_div==2 else [2, 2]
        img_size: Tuple[int, int] = to_2tuple(resolution)
        self.patches_resolution = (img_size[0] // res_div, img_size[1] // res_div)
        ...
        self.seq = nn.Sequential(
            Conv2d_BN(in_chans, n // 2, 3, strides[0], 1),#res /2
            activation(),
            Conv2d_BN(n // 2, n, 3, strides[1], 1),#res /1
        )
...
if __name__ == "__main__":
    ...
    net = tiny_vit_5m_224(pretrained=True)
    ...
    # print("% s seconds" % (m))
    macs, params = profile(net, inputs=(img,)); print("encoder: macs, params: ", macs/(10**9), params)
